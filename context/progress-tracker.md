...
**Last documented:** Financial Reports (Income Statement + Balance Sheet with versioned manual items and PDF export)
...
- [x] Income Statement and Balance Sheet PDFs render every figure inside a fully bordered grid (DESCRIPTION | AMOUNT header band, outer box + row separators + label/amount divider, alternating row tint, accent total rows); Unicode minus/dashes are normalized to a hyphen (`sanitizeText`) so WinAnsi Helvetica renders them cleanly. Layout/borders only — statement math and payloads unchanged
- [x] On-screen Income Statement and Balance Sheet now render as bordered table grids mirroring the PDF (theme-token borders/header band/alternating rows/accent total rules). Income Statement is one `Description | Amount` table; the Balance Sheet keeps its two side-by-side cards but each is one bordered table (Assets; Liabilities & Equity), preserving the comparison column, per-line deltas, and inline manual-item edit/delete. Rendering only — no data, routes, or math touched
- [x] Financial Reports section (admin + manager only): Income Statement (fully auto-derived for a date range, math replicates the Reports page) and Balance Sheet (as-of snapshot). Balance sheet auto lines are reconstructed to the date — Inventory Value (stock rewound by post-date movements, valued at latest receipt cost → sale-cost → costPrice fallback), Accounts Receivable (loan totals minus payments on/before date), Retained Earnings (cumulative income statement). Manual balance-sheet items use an append-only versioned model (`BalanceSheetItem`) so edits/deletes are effective-dated and never alter past snapshots. Both statements export to PDF. Balance Check shows the plain assets − (liabilities + equity) difference; no cash is invented
