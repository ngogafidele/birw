...
| Statement tables | On-screen statements now render as bordered grids that mirror the PDF (`border-collapse border border-border`, every cell `border border-border px-3 py-2`). Header band `bg-muted text-xs uppercase tracking-[0.14em] text-muted-foreground`; `Description` left, `Amount` (or the as-of date when comparing) right. Data line rows zebra with `bg-muted/30` on odd data-row ordinals (band/total rows never break the rhythm — parity is counted from data rows only, no mutable render counter). Subtotal rows `bg-muted/50 font-medium`; grand-total rows `border-t-2 border-t-accent bg-muted/60 font-semibold`. Income Statement Net Profit keeps emerald/rose value coloring by sign; amounts are `text-right tabular-nums` |
...
| Balance sheet layout | `grid items-start gap-4 lg:grid-cols-2`; each side is one bordered `SideTable` in a `rounded-2xl border border-border/80 bg-card p-6 shadow-sm` card — **Assets** (left) and **Liabilities & Equity** (right, a single table combining both sections). Sub-groups (Current/Fixed Assets, Current/Long-term Liabilities, Equity) are full-width `bg-muted` uppercase band rows; empty groups show a `None recorded.` cell; section closers use subtotal (Total Liabilities/Equity) then grand (Total Assets, Total Liabilities + Equity) row styling |
| Comparison column | Optional "Compare To" date inserts a middle column (`Description | <compare date> | <as-of date>`): comparison value `text-right tabular-nums text-muted-foreground`, missing counterpart shows `—`. Subtotal/grand deltas render inside the amount cell as `text-xs font-normal` colored `text-emerald-600` (≥0, `+`-prefixed) or `text-rose-600` (<0). A `text-xs text-muted-foreground` caption states which date is muted and the delta direction |
